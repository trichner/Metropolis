 ...
 */
public class CuboidDAO {

    private final EbeanServer database;

    @Inject
    public CuboidDAO(MetropolisPlugin plugin) {
    ...
    public boolean storeCuboid(Cuboid cuboid){
        if(cuboid==null){
            Minions.w("Invalid cuboid");
            return false;
        }

        CuboidBean bean = new CuboidBean();
        bean.setBlob(cuboid);
    ...
    public boolean containsHash(String hash){
        Query<ClipboardBean> query = database.find(ClipboardBean.class).where().eq("fileHash",hash).query();
    ...
    public List<String> findAllClipboardHashes(){
        Query<ClipboardBean> query = database.find(ClipboardBean.class).select("fileHash");
        List<ClipboardBean> beans = query.findList();
    ...
    public boolean deleteClipboardHashes(String hash){
        Query<ClipboardBean> query = database.find(ClipboardBean.class).where().like("fileHash",hash+"%")
                .query();
        List<ClipboardBean> beans = query.findList();
        List<Integer> ids = getIDs(beans);
        database.delete(ClipboardBean.class, ids);
        return true;
    }
