...
public class FileClipboard implements Clipboard {
    ...
    public Cuboid loadCuboid() {
        try {
            return LibschemAPI.loadSchematic(file);
                    ...
                    if (!rand.getChance(config.getSpawnerOdds())) { //we were unlucky, chest doesn't get placed{
                        ...
                        if (block.getState() instanceof CreatureSpawner) {
                            CreatureSpawner spawner =
                                    (CreatureSpawner) block.getState(); //block has to be a chest
    ...
    private void place(MetropolisGenerator generator, EditSession editSession, Vec3D pos){
        CuboidClipboard cuboid = loadCuboid();
        ...
        for (int x = 0; x < cuboid.getWidth(); x++) {
            for (int y = 0; y < cuboid.getHeight(); y++) {
                for (int z = 0; z < cuboid.getLength(); z++) {
                    ...
                    Material decay = natureDecay
                            .checkBlock(generator.getWorld(), (int) vec.getX(), (int) vec.getY(),
                                    (int) vec.getZ());
    ...
    private int estimateStreetLevel() {
        ...
        for (int y = cuboid.getHeight() - 2; y >= 0; y--) {
            ...
            if (b != Material.AIR.getId() && b != Material.LONG_GRASS.getId() &&
                    b != Material.YELLOW_FLOWER.getId())
                return y + 1;
        }
        return 1;
    }
